...
class up_conv3d(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.up = nn.Sequential(
            nn.ConvTranspose3d(in_ch, in_ch, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1), output_padding=(1, 1, 1)),
            nn.Conv3d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm3d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class U_Net3Dtest(nn.Module):
    ...
    def __init__(self, in_ch=2, out_ch=2):
        super(U_Net3Dtest, self).__init__()
        ...
        self.pre_conv = nn.Sequential(nn.Conv3d(in_ch, 6, kernel_size=3, stride=1, padding=1, bias=True),
                                      nn.BatchNorm3d(6),
                                      nn.ReLU(inplace=True),
                                      nn.Conv3d(6, 6, kernel_size=3, stride=1, padding=1, bias=True),
                                      nn.BatchNorm3d(6),
                                      nn.ReLU(inplace=True),
                                      )

        self.Conv1 = conv_block3d(6, filters[0])
        self.Conv2 = conv_block3d(filters[0], filters[1])
        self.Conv3 = conv_block3d(filters[1], filters[2])
        self.Conv4 = conv_block3d(filters[2], filters[3])
        self.Conv5 = conv_block3d(filters[3], filters[4])

        self.Up5 = up_conv3d(filters[4], filters[3])
        self.Up_conv5 = conv_block3d(filters[4], filters[3])

        self.Up4 = up_conv3d(filters[3], filters[2])
        self.Up_conv4 = conv_block3d(filters[3], filters[2])

        self.Up3 = up_conv3d(filters[2], filters[1])
        self.Up_conv3 = conv_block3d(filters[2], filters[1])

        self.Up2 = up_conv3d(filters[1], filters[0])
        self.Up_conv2 = conv_block3d(filters[1], filters[0])
...
class AttU_Net3Dtest(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=2, output_ch=2):
        super(AttU_Net3Dtest, self).__init__()
        ...
        self.Conv1 = conv_block3d(img_ch, filters[0])
        self.Conv2 = conv_block3d(filters[0], filters[1])
        self.Conv3 = conv_block3d(filters[1], filters[2])
        self.Conv4 = conv_block3d(filters[2], filters[3])
        self.Conv5 = conv_block3d(filters[3], filters[4])
        ...
        self.Up2 = up_conv3d(filters[1], filters[0])
        self.Att2 = Attention_block3D(F_g=filters[0], F_l=filters[0], F_int=32)
        self.Up_conv2 = conv_block3d(filters[1], filters[0])
